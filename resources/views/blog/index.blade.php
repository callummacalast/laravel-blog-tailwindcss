...
@if(session()->has('message'))
    <div class="w-4/5 m-auto-mt-10 pl-2">
        <p class="w-2/6 mb-4 text-grid-50 bg-green-500 rounded-2xl py-4 m-3">
            {{ session()->get('message') }}
        </p>
    </div>
@endif
...
<div class="sm:grid grid-cols-2 gap-20 w-4/5 mx-auto py-15 border-b border-gray-200">
    ...
    <div>
       ...
       <span class="text-gray-500">
            By <span class="font-bold italic text-gray-800">By {{ $post->user->name }}</span>, Created on {{ date('jS M Y', strtotime($post->updated_at)) }}
       </span>
